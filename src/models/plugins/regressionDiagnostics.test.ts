...
const regressionPluginIds = new Set(['vif-analysis', 'partial-correlation', 'hierarchical-regression', 'grouped-regression', 'stepwise-regression'])
...
const regressionFixtures = fixturePack.fixtures.filter((fixture) => regressionPluginIds.has(fixture.pluginId))
...
describe('regression diagnostics plugins', () => {
  regressionFixtures.forEach((fixture) => {
    it(`matches fixture: ${fixture.id}`, () => {
      assertFixture(fixture)
    })
  })

  it('rejects partial correlation when the two variables are identical', () => {
    const plugin = getModelPlugin('partial-correlation')

    expect(() =>
      plugin.fit({
        rows: [{ x: 1 }, { x: 2 }, { x: 3 }, { x: 4 }],
        config: { target: '', features: ['x'], params: { x: 'x', y: 'x', controls: [] } },
      }),
    ).toThrow(/两个不同的数值变量/)
  })
})
